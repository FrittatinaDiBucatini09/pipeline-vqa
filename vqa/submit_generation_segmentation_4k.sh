#!/bin/bash
# ============================================================
# SLURM SUBMISSION SCRIPT: SEGMENTATION 4K VQA GENERATION
# ============================================================
# This script sets up a custom environment to run VQA generation
# using the output from the SEGMENTATION_4K experiment.
#
# It supports evaluating either step 1 (bounding boxes) or 
# step 2 (masks overlays) using a single script.
#
# Usage (Launcher Mode from head node):
#   ./submit_generation_segmentation_4k.sh --step1
#   ./submit_generation_segmentation_4k.sh --step2
#
# When run on the head node, it submits its own execution to SLURM.
# ============================================================

# ----------------------------------------------------
# A. LAUNCHER MODE LOGIC
# ----------------------------------------------------
# If we are NOT running under SLURM, act as a launcher.
if [ -z "$SLURM_JOB_ID" ]; then
    ...
    if [ "$1" == "--step1" ]; then
        ...
        echo "❌ [ERROR] You must specify a target mode:"
        echo "   Usage: $0 --step1 | --step2"
        exit 1
    fi

    # Submit this exact script to SLURM, passing the requested step via an environment variable
    echo "📤 Submitting Segmentation VQA ($STEP) to SLURM..."
    sbatch \
        --job-name=$JOB_NAME \
        --output=slurm_${JOB_NAME}_%j.out \
        --error=slurm_${JOB_NAME}_%j.err \
        -N 1 \
        --gpus=nvidia_geforce_rtx_3090:1 \
        --time=15:00:00 \
        -w faretra \
        --export=ALL,VQA_SEG_STEP=$STEP \
        "$0"
...
# ============================================================
# B. WORKER MODE LOGIC (Running under SLURM)
# ============================================================

echo "============================================================"
echo "          SEGMENTATION 4K VQA GENERATION WORKER"
echo "============================================================"
...
# Determine Input and Output paths based on the requested step.
if [ "$VQA_SEG_STEP" == "step1" ]; then
    echo "🟢 Mode: STEP 1 (Bounding Boxes)"
    INPUT_DIR="$PROJECT_ROOT/orchestrator_runs/SEGMENTATION_4K/step_01_segmentation/results/step1_bboxes"
    OUTPUT_DIR="$PROJECT_ROOT/experiments/03_vqa_segmentation_4k/step1_bboxes"
elif [ "$VQA_SEG_STEP" == "step2" ]; then
    echo "🔵 Mode: STEP 2 (Segmentation Masks)"
    ...
    echo "❌ [ERROR] Invalid or missing VQA_SEG_STEP environment variable."
    exit 1
fi
...
# Configuration File - assume we reuse the attention 4k parameters for now
