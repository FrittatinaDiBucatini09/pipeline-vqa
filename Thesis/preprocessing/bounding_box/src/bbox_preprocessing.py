...
# ==============================================================================
# Robust import strategy to handle custom modules in a cluster environment.
...
def _run_crf_task(task_args: Tuple) -> List[List[int]]:
    ...
    unary = np.zeros((2, proc_size * proc_size), dtype='float32')
    ...
    # 5. Inference
    q_values = d.inference(params['crf_M'])
...
def main():
    ...
    parser.add_argument('--output_format', type=str, choices=['image', 'jsonl'], default='image', help="Serialization format: visual overlays ('image') or structured metadata ('jsonl').")
                ...
                with open(jsonl_path, 'a') as f_json:
                    for i, p in enumerate(paths):
                        ...
                        if args.output_format == 'image':
                            ...
                            # Now points to the generated image in the results folder
                            final_json_image_path = str(relative_save_path)
